...
<div class="container-fluid">
    ...
    <div>
        @forelse($tasks as $task)
            @php
                $latestAttempt = $task->latestAttempt ?? $task->attempts->first();
                $latestReview = $task->latestReview ?? ($latestAttempt ? $latestAttempt->reviews->first() : null);
            @endphp
            <div class="task-card">
                <div class="task-header">
                    <div>
                        ...
                        <div class="task-info">
                            <strong>Tester:</strong> {{ $task->tester->name ?? 'N/A' }}
                        </div>
                        <div class="task-info">
                ...
                @if($latestAttempt)
                    <div class="submission-box">
                        <div class="box-header">Developer Submission</div>
                        <div class="box-content">{{ $latestAttempt->submission_notes }}</div>
                    ...
                    @if($latestReview)
                        <div class="review-box">
                            <div class="box-header">Tester Review</div>
                            <div class="box-content">{{ $latestReview->remarks }}</div>
                            <span class="review-status {{ $latestReview->status }}">
                                {{ ucfirst($latestReview->status) }}
                            </span>
                        </div>
                    @endif

                    <button onclick="openPMReviewModal({{ $task->id }}, '{{ addslashes($task->title) }}')" class="btn-review">
                        ✅ Final Review
                    </button>
                @endif
            </div>
        @empty
            <div class="empty-state">
                ...
                <div class="empty-state-text">No tasks pending PM review</div>
                <p style="color: #6c757d; margin-top: 1rem;">All tasks have been reviewed or no tasks are currently pending your review.</p>
            </div>
        @endforelse
    </div>
</div>
...
<!-- PM Review Modal -->
<div id="pmReviewModal" class="modal-overlay" onclick="if(event.target === this) closePMReviewModal()">
    <div class="modal-content" onclick="event.stopPropagation()">
        <div class="modal-header">
            <h2>Final Review: <span id="pmTaskTitle"></span></h2>
        </div>
        
        <form id="pmReviewForm" method="POST">
            @csrf
            @if($errors->any())
            ...
            <div class="form-group">
                ...
                <div>
                    <label class="radio-option">
                        <input type="radio" name="status" value="correct" required>
                        <div class="radio-label">
                            <div class="radio-title correct">✓ Correct</div>
                            <div class="radio-description">Task is correct and complete (+10 points, status → final_completed)</div>
                        </div>
                    </label>
                    <label class="radio-option">
                        <input type="radio" name="status" value="issue" required>
                        <div class="radio-label">
                            <div class="radio-title issue">✗ Issue Found</div>
                            <div class="radio-description">Task needs revision (-5 points, sent back to developer)</div>
                        </div>
                    </label>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="remarks" rows="6" required
                    class="form-control"
                    placeholder="Provide your final review and feedback..."></textarea>
                <div class="form-helper">Explain your decision and provide any final feedback</div>
            </div>
            ...
            <div class="modal-actions">
                ...
                <button type="button" onclick="closePMReviewModal()" class="btn-cancel">
                    Cancel
                </button>
            </div>
        </form>
...
<script>
function openPMReviewModal(taskId, taskTitle) {
    document.getElementById('pmTaskTitle').textContent = taskTitle;
    document.getElementById('pmReviewForm').action = `/pm/task/${taskId}/review`;
    document.getElementById('pmReviewModal').classList.add('show');
    document.body.style.overflow = 'hidden';
}

function closePMReviewModal() {
    document.getElementById('pmReviewModal').classList.remove('show');
    document.getElementById('pmReviewForm').reset();
