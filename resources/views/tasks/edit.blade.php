...
<div class="container-fluid">
    ...
    <div class="page-header">
        <h1>✏ Edit Task</h1>
        <p>Update task details or reassign users</p>
    </div>

    <!-- Form Card -->
    <div class="form-card">
        @if ($errors->any())
            <div class="alert alert-danger" style="background: rgba(220,53,69,0.1); border-color: #dc3545; color: #721c24; padding: 1.25rem; border-radius: 12px; margin-bottom: 2rem;">
                <div style="flex: 1;">
                        ...
                        @if($task->failed_attempts >= 5)
                            <div class="alert-warning">
                                <p style="margin: 0; font-size: 0.9rem;">
                                    ⚠️ This task has {{ $task->failed_attempts }} failed attempts. Reassigning will reset the status to pending.
                                </p>
                            </div>
                        @endif
                        <div class="form-group" style="margin-bottom: 0 !important;">
                            <select 
                                ...
                                @foreach($developers as $dev)
                                    <option value="{{ $dev->id }}" {{ old('developer_id', $task->developer_id) == $dev->id ? 'selected' : '' }}>
                                        {{ $dev->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group" style="margin-bottom: 0 !important;">
                            <select 
                                ...
                                @foreach($testers as $tester)
                                    <option value="{{ $tester->id }}" {{ old('tester_id', $task->tester_id) == $tester->id ? 'selected' : '' }}>
                                        {{ $tester->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="team-card pm">
                        ...
                        <div class="form-group" style="margin-bottom: 0 !important;">
                            <select 
                                ...
                                @foreach($pms as $pm)
                                    <option value="{{ $pm->id }}" {{ old('pm_id', $task->pm_id) == $pm->id ? 'selected' : '' }}>
                                        {{ $pm->name }}
                                    </option>
                                @endforeach
                            </select>
